...
class RobotSensorInterfaceBridge:
    ...
    def __init__(self, local_ip: str = "192.168.1.4", local_port: int = 59152):
        ...
        # self.RE_EEF = re.compile(r'<EEF\s+ServoM="([^"]+)"')
    ...
    def _unpack_incoming(self, xml_text: str) -> int:
        ...
        # 만약 EEF ServoMotor, Spindle, I/O, Axis_position 등의 데이터 또한 KUKA컨트롤러를 통해 전송될 경우 unpacking 필요
        # m_eef = self.RE_EEF.search(xml_text)
        ...
        with self._sub_lock:
            ...
            # if m_eef:
            #     self.data_sub["EEF_ServoM"] = float(m_eef.group(?))
